// Solution that uses the engine from Solution 1 but implements recursion
function pascalRow(n) {
  ...
  return calculateNextRow([1, 1], n);
}

const calculateNextRow = (previousRow, n) => {
  const currentRow = [1];
  ...
  if (currentRow.length < n)
    return calculateNextRow(currentRow, n);
  else
    return currentRow;
};
