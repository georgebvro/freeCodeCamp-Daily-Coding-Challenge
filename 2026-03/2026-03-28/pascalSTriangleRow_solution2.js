...
function pascalRow(n) {
  ...
  while (currentRow.length < n) {
    ...
    for (let i = 0; i <= previousRow.length / 2; ++i) {
      currentRow.push((previousRow[i - 1] || 0) + previousRow[i]);
    }

    currentRow = currentRow.concat(currentRow.toReversed().slice((previousRow.length - 1) % 2));
  }
